...
export function getConfig(): AppConfig {
  ...
  const config: AppConfig = {
    ...
    cookieSecure:
      process.env.COOKIE_SECURE === "true" || DEFAULT_CONFIG.cookieSecure,
    defaultLanguage:
      process.env.DEFAULT_LANGUAGE || DEFAULT_CONFIG.defaultLanguage,
    forceItalian:
      process.env.FORCE_ITALIAN === "true" || DEFAULT_CONFIG.forceItalian,
  };
...
function getDatabaseConfig(): DatabaseConfig {
  const dbType = (process.env.DB_TYPE ||
    DEFAULT_CONFIG.database.type) as DatabaseType;

  // Configurazione per SQLite (default)
  if (dbType === "sqlite") {
    return {
      type: "sqlite",
      path: process.env.SQLITE_DB_PATH || DEFAULT_CONFIG.database.path,
    };
  }
  ...
  console.warn(
    `Tipo di database "${dbType}" non supportato. Utilizzo di SQLite.`
  );
  return {
    type: "sqlite",
  };
